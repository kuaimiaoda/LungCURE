...
DEFAULT_OUTPUT_FILE = os.getenv(
    "OUTPUT_FILE",
    os.path.join(DEFAULT_BENCHMARK_DIR, "my_framework", "results_TNMstaging", "results_LLM", f"{RUNNER_MODEL.split('/')[-1]}_Chinese_simplified.json"),
)
...
def build_runner_messages(images: List[Any]) -> List[Dict[str, Any]]:
    ...
    prompt = """
你是一名肺癌 TNM 分期专家。请根据报告内容，输出简化版 TNM 结果。

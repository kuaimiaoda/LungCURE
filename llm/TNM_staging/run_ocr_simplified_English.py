...
DEFAULT_OCR_ROOT = os.getenv("OCR_ROOT", os.path.join(DEFAULT_BENCHMARK_DIR, "470_md"))
DEFAULT_OCR_SEED = int(os.getenv("OCR_SEED", "42"))
DEFAULT_OUTPUT_FILE = os.getenv(
    "OUTPUT_FILE",
    os.path.join(DEFAULT_BENCHMARK_DIR, "my_framework", "results_TNMstaging", "results_ocr_LLM", f"{RUNNER_MODEL.split('/')[-1]}_English_ocr_simplified.json"),
)
MAX_RUNNER_ATTEMPTS = max(1, int(os.getenv("MAX_RUNNER_ATTEMPTS", "1")))
...
def normalize_simplified_output(candidate: Dict[str, Any]) -> Dict[str, str]:
    ...
    out["Final_TNM"] = " ".join([p for p in [out["T_stage"], out["N_stage"], out["M_stage"]] if p])
    return out
...
# 3. T_stage must be one of Tx/T0/T1a/T1b/T1c/T2a/T2b/T3/T4.
# 4. N_stage must be one of Nx/N0/N1/N2/N3.
# 5. M_stage must be one of M0/M1a/M1b/M1c.
def build_runner_messages(ocr_text: str) -> List[Dict[str, Any]]:
    prompt = f"""
You are an lung-cancer TNM staging expert.
Below is an OCR-converted markdown report. Based on this report text, output only a simplified TNM JSON.

Strict requirements:
1. Output exactly one JSON object and no extra text.
2. The JSON must contain exactly these 7 non-empty string fields:
   - T_stage
   - T_reasoning
   - N_stage
   - N_reasoning
   ...
   - M_reasoning
   - Final_TNM
3. T_stage must have a valid staging value.
4. N_stage must have a valid staging value.
5. M_stage must have a valid staging value.
6. Final_TNM must explicitly include T_stage, N_stage, and M_stage.
7. All fields must be written in English only. Do not use Chinese characters.
...
""".strip()
    ...
    return [
        {"role": "system", "content": "You are a strict clinical reasoning model that must follow the output schema exactly."},
...
def build_judge_messages(ocr_text: str, gt_simplified: Dict[str, str], pred_simplified: Dict[str, str]) -> List[Dict[str, Any]]:
    ...
    judge_prompt = f"""
You are a strict lung-cancer TNM benchmark judge. Evaluate only based on the OCR markdown text below, GT_Simplified, Pred_Simplified, and AJCC 8th-edition lung-cancer TNM rules. The OCR text is only for checking whether Pred contains obvious contradictions with the case content; scoring should still primarily follow GT_Simplified versus Pred_Simplified.
...
""".strip()
    ...
    return [
        {"role": "system", "content": "You are a strict and objective medical evaluator. Output valid JSON only."},
        {"role": "user", "content": judge_prompt},
    ]


def call_runner_until_valid(client: OpenAI, messages: List[Dict[str, Any]]) -> Dict[str, str]:
    ...
    for attempt in range(1, MAX_RUNNER_ATTEMPTS + 1):
        try:
            ...
            if not parsed:
                last_error = "runner output is not valid JSON"
                continue
            normalized = normalize_simplified_output(parsed)
    ...
    raise RuntimeError(f"Runner failed after {MAX_RUNNER_ATTEMPTS} attempts: {last_error}")
...
def main() -> None:
    ...
    parser.add_argument("--ocr-root", type=str, default=DEFAULT_OCR_ROOT)
    parser.add_argument("--ocr-dir", type=str, default="")
    parser.add_argument("--ocr-seed", type=int, default=DEFAULT_OCR_SEED)
    ...
    todo = [(cid, item) for cid, item in selected if cid not in results or not isinstance(results.get(cid), dict) or "error" in results.get(cid, {})]
    ...
    for case_id, item in tqdm(todo):
        ...
        try:
            ...
            runner_msgs = build_runner_messages(ocr_text)
            pred_simplified = call_runner_until_valid(runner_client, runner_msgs)

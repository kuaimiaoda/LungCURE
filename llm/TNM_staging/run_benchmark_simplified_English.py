...
DEFAULT_OUTPUT_FILE = os.getenv(
    "OUTPUT_FILE",
    os.path.join(DEFAULT_BENCHMARK_DIR, "my_framework", "results_TNMstaging", "results_LLM", f"{RUNNER_MODEL.split('/')[-1]}_English_simplified.json"),
)
...
def extract_json(text: str) -> Optional[Dict[str, Any]]:
    ...
    # Fallback: parse line-style key-value outputs like `T_stage: T2a`
    kv: Dict[str, str] = {}
...
def normalize_simplified_output(candidate: Dict[str, Any]) -> Dict[str, str]:
    ...
    # Canonicalize Final_TNM from normalized stage tokens to avoid format drift.
...
def build_runner_messages(images: List[Any]) -> List[Dict[str, Any]]:
    ...
    prompt = """
You are a lung-cancer TNM staging expert.
Given the report images, output only a simplified TNM JSON.
...
""".strip()
    ...
    return [
        {
            "role": "system",
            "content": "You are a strict clinical reasoning model that must follow output schema exactly.",
        },
...
def build_judge_messages(images: List[Any], gt_simplified: Dict[str, str], pred_simplified: Dict[str, str]) -> List[Dict[str, Any]]:
    ...
    judge_prompt = f"""
You are a strict lung-cancer TNM benchmark judge. Evaluate only based on GT_Simplified, Pred_Simplified, and AJCC 8th-edition lung-cancer TNM rules. You may use the report images only to cross-check obvious contradictions, but scoring should primarily follow GT_Simplified versus Pred_Simplified.
...
def main() -> None:
    ...
    for case_id, item in tqdm(todo):
        ...
        try:
            runner_msgs = build_runner_messages(images)

...
DEFAULT_OUTPUT_FILE = os.getenv(
    "OUTPUT_FILE",
    os.path.join(DEFAULT_BENCHMARK_DIR, "my_framework", "results_TNMstaging", "results_ocr_LLM", f"{RUNNER_MODEL.split('/')[-1]}_Chinese_ocr_simplified.json"),
)
...
def resolve_ocr_dir(benchmark_dir: str, ocr_root: str, seed: int, ocr_dir: str) -> str:
    if ocr_dir:
        return ocr_dir
    if ocr_root:
        return os.path.join(ocr_root, f"OCR_Chinese_seed{seed}")
    return os.path.join(benchmark_dir, "470_md", f"OCR_Chinese_seed{seed}")
...
# 3. T_stage 只能是类似 Tx/T0/T1a/T1b/T1c/T2a/T2b/T3/T4。
# 4. N_stage 只能是 Nx/N0/N1/N2/N3。
# 5. M_stage 只能是 M0/M1a/M1b/M1c。
def build_runner_messages(ocr_text: str) -> List[Dict[str, Any]]:
    prompt = f"""
你是一名肺癌 TNM 分期专家。下面给你的是已经完成 OCR 的病例报告 markdown 文本，请根据报告内容，输出简化版 TNM 结果。

严格要求：
1. 只输出一个 JSON 对象，不要输出任何额外文字。
2. JSON 必须严格包含以下 7 个字段且都是非空字符串：
   - T_stage
   - T_reasoning
   - N_stage
   - N_reasoning
...
4. N_stage 必须有一个分期结果。
5. M_stage 必须有一个分期结果。
6. Final_TNM 必须显式包含 T_stage、N_stage、M_stage 三者。

病例 OCR markdown 文本如下：
```markdown
{ocr_text}
```
...
""".strip()
    ...
    return [
        {"role": "system", "content": "你是严格遵循输出模式的临床推理模型。"},
...
def build_judge_messages(ocr_text: str, gt_simplified: Dict[str, str], pred_simplified: Dict[str, str]) -> List[Dict[str, Any]]:
    ...
    judge_prompt = f"""
你是肺癌 TNM benchmark 的严格评测裁判。请只基于以下 OCR 文本、GT_Simplified、Pred_Simplified 以及 AJCC 第八版肺癌 TNM 分期规则进行评估。OCR 文本仅用于核对 Pred 是否出现与原始病例内容明显矛盾的情况，但评分仍以 GT_Simplified 和 Pred_Simplified 为主。
...
""".strip()
    ...
    return [
        {"role": "system", "content": "你是严格客观的医学评测裁判，只输出合法 JSON。"},

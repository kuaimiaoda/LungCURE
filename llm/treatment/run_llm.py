...
def _rewrite_path_for_project(value: str, *, project_root: Path, treatment_root: Path) -> str:
    ...
    for cand in candidates:
        if cand and (treatment_root / cand).exists():
            return str(Path("llm") / "treatment" / cand).replace("\\", "/")
...
def main() -> None:
    ...
    target = (treatment_root / "code" / "inference" / "direct_llm_batch_infer.py").resolve()
    ...
    args = _normalize_path_flags(
        ...
        flags={"--benchmark-dir", "--case-file", "--output-json"},
    )
    ...
    if not _has_flag(args, "--output-json"):
        ...
        if case_file:
            out_name = f"{_safe_output_stem(case_file, 'case')}_llm_results.json"
        else:
            out_name = "llm_results.json"
        out_rel = Path("llm") / "outputs" / out_name

...
DEFAULT_OUTPUT_FILE = os.getenv(
    "OUTPUT_FILE",
    os.path.join(DEFAULT_BENCHMARK_DIR, "my_framework", "results_TNMstaging", "results_ocr_Agent", f"{RUNNER_MODEL.split('/')[-1]}_Chinese_agent_ocr_simplified.json"),
)
...
DIFY_WORKFLOW_FILE = os.getenv("DIFY_WORKFLOW_FILE", os.path.join(DEFAULT_BENCHMARK_DIR, "my_framework", "LCAgent", "TNM_staging", "tnm分期助手_分开_格式化_20260327.yml"))
...
def build_judge_messages(ocr_text: str, gt_simplified: Dict[str, str], pred_simplified: Dict[str, str]) -> List[Dict[str, Any]]:
    ...
    judge_prompt = f"""
你是肺癌 TNM benchmark 的严格评测裁判。请只基于以下病例 OCR markdown 文本、GT_Simplified、Pred_Simplified 以及 AJCC 第八版肺癌 TNM 分期规则进行评估。OCR 文本仅用于核对 Pred 是否存在与原报告明显矛盾的情况，但评分仍以 GT_Simplified 和 Pred_Simplified 为主。

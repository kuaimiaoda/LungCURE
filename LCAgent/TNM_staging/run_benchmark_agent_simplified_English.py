...
DEFAULT_OUTPUT_FILE = os.getenv(
    "OUTPUT_FILE",
    os.path.join(DEFAULT_BENCHMARK_DIR, "my_framework", "results_TNMstaging", "results_Agent", f"{RUNNER_MODEL.split('/')[-1]}_English_agent_simplified.json"),
)
...
DIFY_WORKFLOW_FILE = os.path.join(DEFAULT_BENCHMARK_DIR, "my_framework", "LCAgent", "TNM_staging", "tnm_staging_assistant_en_0327.yml")
...
def build_agent_extract_messages(images: List[Any]) -> List[Dict[str, Any]]:
    ...
    title_candidates = ["Extract Info", "提取信息"]
    user_template = get_dify_prompt_with_fallback(title_candidates, "user")
    user_text = render_dify_template(
        user_template,
        {
            "{{#sys.query#}}": "",
            "{{#1756804402464.text#}}": "Please extract key information from the uploaded report images.",
        },
    )
...
def build_agent_t_split_messages(extracted_text: str) -> List[Dict[str, Any]]:
    title_candidates = ["T Routing", "T提取"]
    user_template = get_dify_prompt_with_fallback(title_candidates, "user")
